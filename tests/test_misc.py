...
def test_get_address_unspent_uses_unspent_endpoint(monkeypatch):
    ...
    captured = []
    monkeypatch.setattr(requests, 'get', lambda url: (captured.append(url), fake)[1])
    from yubtc.misc import get_address_unspent
...
def test_get_address_unspent_propagates_non_json_errors(monkeypatch):
    """Pin the dead-code bug: a KeyError is not JSONDecodeError, so it
    propagates and the `raise Exception('Unknown error')` after the
    try/except is never reached."""
    import requests
    fake = MagicMock()
...
def test_get_address_info_propagates_non_json_errors(monkeypatch):
    """Same dead-code bug as get_address_unspent."""
    import requests
    fake = MagicMock()
